...
# Maps genetic variants to the Alu Editing Index (AEI) as a **genome-wide trans-QTL** phenotype.
...
FASTQTL_BIN="/path/to/FastQTL/executable" # <-- ENSURE THIS PATH IS CORRECTLY SET
...
# Mapping Parameters 
# For AEI-QTL (trans-QTL), we use the --trans flag.
TRANS_WINDOW_SIZE=10000000 # Use a large window (e.g., 10Mb) for --trans flag
PERMUTATIONS=1000
...
# This script should generate the AEIQTL_PHENOTYPE (1 feature) and AEIQTL_COVARIATES.
python3 ${PREP_SCRIPT} \
...
echo "--- Starting FastQTL for Genome-Wide AEI-QTL Mapping (Trans-QTL) ---"

# 2. Execution: FastQTL
# **CRITICAL CHANGE:** Replacing --window with --trans.
# The --trans flag performs a genome-wide scan, suitable for the non-positional AEI phenotype.
${FASTQTL_BIN} \
    --vcf ${GENOTYPE_FILE} \
    --bed ${AEIQTL_PHENOTYPE} \
    --cov ${AEIQTL_COVARIATES} \
    --output ${FINAL_OUTPUT_FILE} \
    --trans ${TRANS_WINDOW_SIZE} \ # <-- Trans-QTL mapping using a large exclusion window (10Mb)
