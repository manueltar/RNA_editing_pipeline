#!/bin/bash
#
# SLURM Script for Phase 4 (Reworked) v3: Quantification with Optional Annotation Filtering
#
# Queries the 31 BAMs using the full Phase 3 matrix, applies final filters, and annotates.
#

# === SLURM Directives ===
#SBATCH --job-name=P4_Quant_Filter_v3
#SBATCH --nodes=1
...
#SBATCH --output=logs/p4_quant_v3_%j.out
#SBATCH --error=logs/p4_quant_v3_%j.err
...
# --- Define Paths ---
QUANT_PYTHON_SCRIPT="./Python_scripts/run_phase4_quantification_v3.py"
...
# Input Master Site Matrix (Phase 3 output)
...
# Final Output Matrix
QUANTIFICATION_MATRIX="${OUTPUT_DIR}/${INDIVIDUAL_ID}_final_editing_matrix_P4_KnownUTR3.tsv"
...
# --- NEW ANNOTATION FILTER PARAMETERS ---
# To enable strict filtering (Known & UTR3):
FILTER_REDIP_STATUS="Known"
FILTER_FUNC_REGION="UTR3"
# To disable filtering, set them to empty strings or don't include the arguments below.

mkdir -p logs
mkdir -p ${OUTPUT_DIR}

echo "Starting Phase 4 (v3): Quantification and Filtering for ${INDIVIDUAL_ID}..."
...
python3 ${QUANT_PYTHON_SCRIPT} \
    ...
    --threads ${THREADS} \
    --filter_redip_status "${FILTER_REDIP_STATUS}" \
    --filter_func_region "${FILTER_FUNC_REGION}"
...
echo "Phase 4 (v3) complete. Quantification matrix saved to ${QUANTIFICATION_MATRIX}"
